    ...
    <div class="w-full px-16 py-20 mt-6 overflow-hidden bg-white rounded-lg lg:max-w-4xl"> -->
      ...
      <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
        <!-- <form method="POST" action="#"> -->
          ...
          <div>
            <div class="">
              <img class="object-cover md:object-scale-down w-5/12 object-center" src="{{ asset('img/portfolio/'.$portfolios->image_url) }}">
            </div>
            <h3 class="text-2xl font-semibold">{{ $portfolios->title }}</h3>
            <div class="flex items-center mb-4 space-x-2">
              <span class="text-xs text-gray-500"> {{ $portfolios->created_at }}</span><span class="text-xs text-gray-500">Created by
                {{ $portfolios->writer }}</span>
            </div>
            <p class="text-base text-gray-700">{{$portfolios->body}}</p>
            <a href="{{$portfolios->link}}" class="text-base text-gray-700">link</a>
          </div>
        <!-- </form> -->
      </div>
    </div>
